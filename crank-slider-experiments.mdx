...
:::tip[Instrument it]
A slider-crank lives in engines, pumps, and compressors, and it fails in known ways: bearing and wrist-pin wear, piston side-thrust scoring the cylinder, and combustion knock. All of it shows up as vibration (a car's knock sensor is just an accelerometer on the block). Mount a MEMS accelerometer, an ADXL345 or the lower-noise ADXL355, near the main bearing; in a pump or compressor add a pressure transducer at the cylinder head to watch the actual delivery. An [ESP32 or STM32](/education/sensor-actuator-interfacing-stm32/) reads them, and on a small board it can [stream](/education/iot-systems/) the trace to tune the machine and [flag the anomaly](https://siliconwit.io) before a bearing lets go.
:::
...
With r = 50 mm and l = 150 mm, find the strongest quick-return you can reach by raising the offset to the slider's limit of e = 50 mm, and read the ratio in the time domain (it is only about 1.2). Now reason about why an offset slider-crank cannot reach the 1.7-plus ratio a real shaper needs: the offset must stay below l - r for the mechanism to assemble, and the stroke stays close to 2r, so the asymmetry it can produce is limited. This ceiling is exactly why production shapers use the **crank-and-slotted-lever** mechanism instead, worked through in the [planar-mechanics quick-return application](/education/planar-mechanics/position-analysis-planar-linkages#application-5-quick-return-position-a-slider-crank-inversion). For a ready-made strong case here, the **Quick-Return (offset)** preset (r = 60, l = 120, e = 50) reaches a ratio of about 1.6.
...
:::note[Objective]
Compare the three real-world application presets side by side, plus the quick-return demonstration, and identify which parameter choices serve which engineering goals.
:::
...
5. **Quick-Return preset**
   Select Quick-Return (offset) (r=60, l=120, e=50, 40 RPM, F=300 N). Its large offset makes the velocity profile the most asymmetric of the four: the forward and return strokes take clearly unequal times (ratio about 1.6).
...
| Measurement | IC Engine | Air Compressor | Hydraulic Press | Quick-Return |
|-------------|-----------|----------------|-----------------|--------------|
| Stroke (mm) | | | | |
| l/r ratio | | | | |
| Peak velocity (mm/s) | | | | |
