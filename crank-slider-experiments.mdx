...
For each experiment, you will read values from the simulator's charts and summary panels, then save them as a CSV file. You can create these CSV files in any text editor or spreadsheet application.

**Example:** For Experiment 1, create `data/exp1_baseline.csv` with the values you read:

```text title="data/exp1_baseline.csv"
angle_deg,displacement_mm,velocity_mm_s,acceleration_mm_s2
0,200.00,0.00,-2631.89
30,186.60,-200.12,-1975.43
60,153.45,-298.77,-450.21
90,141.42,-314.16,697.89
```

Read values by hovering over the charts at the angles listed in each experiment's data collection table. Alternatively, you can download a complete CSV dataset from the simulator's download section.

The Python scripts in each experiment can also generate analytical data independently, so you can compare your recorded readings against the exact mathematical solution.
...
ma = np.where(np.abs(dxdtheta) > 0.001, 1 / np.abs(dxdtheta), 0)
torque = F * (v / 1000) / omega

fig, axes = plt.subplots(3, 1, figsize=(10, 9), sharex=True)

axes[0].plot(theta, v, 'g-', linewidth=2)
...
axes[1].plot(theta, ma, 'm-', linewidth=2)
axes[1].set_ylabel('Mechanical Advantage')
axes[1].set_ylim(0, 0.15)  # Clip the infinity spikes for readability
axes[1].axvline(x=0, color='r', linestyle='--', alpha=0.5)
axes[1].axvline(x=180, color='b', linestyle='--', alpha=0.5)

axes[2].plot(theta, torque, 'r-', linewidth=2)
axes[2].set_ylabel('Crank Torque (Nm)')
axes[2].set_xlabel('Crank Angle (degrees)')
axes[2].axhline(y=0, color='k', linewidth=0.5)
axes[2].axvline(x=0, color='r', linestyle='--', alpha=0.5)
axes[2].axvline(x=180, color='b', linestyle='--', alpha=0.5)

plt.tight_layout()
plt.savefig('experiment_5_dead_centers.png', dpi=150)
plt.show()

# Print values at key angles
for angle in [0, 30, 60, 90, 120, 150, 180]:
    print(f"theta={angle:3d}: x={x[angle]:.2f} mm, v={v[angle]:.2f} mm/s, MA={ma[angle]:.4f}, T={torque[angle]:.4f} Nm")
```
